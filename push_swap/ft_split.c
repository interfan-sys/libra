...
/*   Updated: 2026/04/16 12:31:46 by agkicina         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	char	**alloc_result(const char *str)
{
	char	**res;
...
static	void	ft_free_split(char **res, int j)
{
	while (j >= 0)
		free(res[j--]);
	free(res);
...
char	**ft_split(const char *str)
{
	...
	while (str[i])
	{
		...
		if (!res[j])
			return (ft_free_split(res, j - 1), NULL);
		j++;
	}
